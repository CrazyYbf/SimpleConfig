...
const char *mcast_nonce_buffer="8CmT/ J(3_aE R_UFR}`mtwF=)Qfjtn^S_1/ffg<_C7yw's}?'_'n&2~Blm&_k?6";
unsigned char default_key_iv[] = {0xA6,0xA6,0xA6,0xA6,0xA6,0xA6,0xA6,0xA6,0xA6};
#define BLKSIZE8                    (8)
#define BLKSIZE                     (16)
...
- (id)init: (unsigned int)pattern_flag
{
    ...
    NSLog(@"init: &m_configSocket=%p", &m_configSocket);
...
- (int)generate_key
{
    ...
    memset(buffer, '\0', 128);
    const char *tmp;
    NSString *default_pin = PATTERN_TWO_DEF_PIN;
...
- (int)encrypt_profile
{
    ...
    memset(_x, 0x0, 32);
    memcpy(A, default_key_iv, BLKSIZE8);
    for(i=0; i<nblk; i++)
...
-(int)updateConfigList: (unsigned char *)data_p len:(unsigned int)data_length
{
    ...
    NSValue *old_dev_val;
    int dev_total_num = [m_config_list count];
    ...
    if (exist) {
        ...
        if (old_dev.ip!=0) {
            ...
            [self rtk_sc_send_ack_packets:old_dev.ip];
        }else{
            // ack-ack1, too many from client, just reply multicast
            getIP = 0;
            [self rtk_sc_send_ack_packets];
        }
...
AddNewObj:
    {
        ...
        if (new_dev.ip==0){
            ...
            getIP = 1;
            [self rtk_sc_send_ack_packets:new_dev.ip];
        }
...
#if PATTERN_TWO_DBG
    // for debug
    NSLog(@"data in udp is %d bytes: ", [data length]);
...
- (int)rtk_sc_send_ack_packets
{
    ...
    NSString *pin = PATTERN_TWO_DEF_PIN;

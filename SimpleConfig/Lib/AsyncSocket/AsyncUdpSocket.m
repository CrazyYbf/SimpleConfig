...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket4)
    {
        ...
        setsockopt(CFSocketGetNative(theSocket4), SOL_SOCKET, SO_REUSEPORT, &reuseOn, sizeof(reuseOn));
    }
	if (theSocket6)
    {
        setsockopt(CFSocketGetNative(theSocket6), SOL_SOCKET, SO_REUSEADDR, &reuseOn, sizeof(reuseOn));
        setsockopt(CFSocketGetNative(theSocket6), SOL_SOCKET, SO_REUSEPORT, &reuseOn, sizeof(reuseOn));
    }
